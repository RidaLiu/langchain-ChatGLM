...
def get_answer(
    ...
    if mode == "Bing搜索问答":
        ...
        mode == "知识库问答"
        and vs_path is not None
        and os.path.exists(vs_path)
        ...
        for answer_result in local_doc_qa.llm.generatorAnswer(
            prompt=query, history=history[:-1], streaming=streaming
        ):
    ...
    flag_csv_logger.flag(
        [query, vs_path, history, mode], username=FLAG_USER_NAME
    )
...
def get_vector_store(
    vs_id, files, sentence_size, history, one_conent, one_content_segmentation
):
    ...
    if local_doc_qa.llm and local_doc_qa.embeddings:
        if isinstance(files, list):
            for file in files:
                ...
                shutil.move(
                    file.name, os.path.join(UPLOAD_ROOT_PATH, vs_id, filename)
                )
...
def reinit_vector_store(vs_id, history):
    try:
        ...
        vs_path, loaded_files = local_doc_qa.init_knowledge_vector_store(
            UPLOAD_ROOT_PATH, vs_path, sentence_size
        )
...
with gr.Blocks(
    css=block_css, theme=gr.themes.Default(**default_theme_args)
) as demo:
    vs_path, file_status, model_status = (
        gr.State(
            os.path.join(VS_ROOT_PATH, get_vs_list()[0])
            if len(get_vs_list()) > 1
            else ""
        ),
        gr.State(""),
        ...
        with gr.Row():
            ...
            with gr.Column(scale=5):
                ...
                with vs_setting:
                    ...
                    vs_refresh.click(
                        fn=refresh_vs_list, inputs=[], outputs=select_vs
                    )
                    vs_add.click(
                        fn=add_vs_name,
            ...
            with gr.Column(scale=5):
                ...
                with knowledge_set:
                    ...
                    chunk_conent = gr.Checkbox(
                        value=False, label="是否启用上下文关联", interactive=True
                    )
                    ...
                    chunk_conent.change(
                        ...
                        inputs=[
                            chunk_conent,
                            gr.Textbox(value="chunk_conent", visible=False),
                            chatbot,
                        ],
                    ...
                    with file2vs:
                        ...
                        with gr.Tab("添加单条内容"):
                            ...
                            one_content_segmentation = gr.Checkbox(
                                value=True, label="禁止内容分句入库", interactive=True
                            )
    ...
    with gr.Tab("模型配置"):
        ...
        use_ptuning_v2 = gr.Checkbox(
            USE_PTUNING_V2, label="使用p-tuning-v2微调过的模型", interactive=True
        )
        use_lora = gr.Checkbox(USE_LORA, label="使用lora微调的权重", interactive=True)
